...
type
 ...
 TTrMng = class(TForm)
   ...
   procedure Edit6_7KeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
 ...
 with CurrentWindow do
  begin
   ...
   if TrOp = 0 then
    begin
     New(OldPat);
     OldPat^ := VTMP^.Patterns[TPat]^;
    end
   else if not AcceptCannotUndo(Mes_UndoThisOp) then
     Exit;
    ...
    begin
     ...
     if TrMng.CheckBox1.Checked then
      begin
       ...
       case TrOp of
         ...
         2: begin
           VTMP^.Patterns[FPat]^.Items[i + FLin].Envelope :=
             VTMP^.Patterns[TPat]^.Items[i + TLin].Envelope;
           VTMP^.Patterns[TPat]^.Items[i + TLin].Envelope := j;
          end
        end;
      end;
     if TrMng.CheckBox2.Checked then
      begin
       ...
       case TrOp of
         ...
         2: begin
           VTMP^.Patterns[FPat]^.Items[i + FLin].Noise :=
             VTMP^.Patterns[TPat]^.Items[i + TLin].Noise;
           VTMP^.Patterns[TPat]^.Items[i + TLin].Noise := j;
          end
        end;
      end;
     ...
     case TrOp of
       ...
       1: begin
         ...
         VTMP^.Patterns[FPat]^.Items[i + FLin].Channel[FChn].Additional_Command.Number
         := 0;
         VTMP^.Patterns[FPat]^.Items[i + FLin].Channel[FChn].Additional_Command.Delay := 0;
         VTMP^.Patterns[FPat]^.Items[i + FLin].Channel[FChn].Additional_Command.Parameter
         := 0;
        end;
       2: begin
         VTMP^.Patterns[FPat]^.Items[i + FLin].Channel[FChn] :=
           VTMP^.Patterns[TPat]^.Items[i + TLin].Channel[TChn];
         VTMP^.Patterns[TPat]^.Items[i + TLin].Channel[TChn] := cl;
        end
      end;
    end;
    ...
    begin
     ...
     if TrOp = 0 then
      begin
       if TrMng.CheckBox1.Checked then
         i := 3
       else if TrMng.CheckBox2.Checked then
         i := 6
       else
         i := TChn * 14 + 8;
       AddUndo(CATracksManagerCopy,{%H-}PtrInt(OldPat), 0, TPat, TLin, TChn, i);
      end;
    end
   else if TrOp = 0 then
     Dispose(OldPat);
...
begin
 MainForm.GlueFormToControl(Self, MainForm.TBTracksMngr);
end;

procedure TTrMng.FormKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
begin
...
begin
 TracksOp(UpDown3.Position, UpDown4.Position, UpDown7.Position, UpDown1.Position,
   UpDown2.Position, UpDown6.Position, 0);
end;

procedure TTrMng.SpeedButton2Click(Sender: TObject);
begin
 TracksOp(UpDown1.Position, UpDown2.Position, UpDown6.Position, UpDown3.Position,
   UpDown4.Position, UpDown7.Position, 0);
end;

procedure TTrMng.SpeedButton3Click(Sender: TObject);
begin
 TracksOp(UpDown3.Position, UpDown4.Position, UpDown7.Position, UpDown1.Position,
   UpDown2.Position, UpDown6.Position, 1);
end;

procedure TTrMng.SpeedButton4Click(Sender: TObject);
begin
 TracksOp(UpDown1.Position, UpDown2.Position, UpDown6.Position, UpDown3.Position,
   UpDown4.Position, UpDown7.Position, 1);
end;

procedure TTrMng.SpeedButton5Click(Sender: TObject);
begin
 TracksOp(UpDown1.Position, UpDown2.Position, UpDown6.Position, UpDown3.Position,
   UpDown4.Position, UpDown7.Position, 2);
end;
...
begin
 if not MainForm.ActiveChildExists then
   Exit;
 Chans[0] := False;
 Chans[1] := False;
 Chans[2] := False;
 Chans[Chn] := True;
 MainForm.TransposeColumns(MainForm.ActiveChild, Pat, CheckBox1.Checked,
...
procedure TTrMng.UpDown6_7ChangingEx(Sender: TObject; var AllowChange: boolean;
 NewValue: smallint; Direction: TUpDownDirection);
begin
...
procedure TTrMng.Edit6_7KeyDown(Sender: TObject; var Key: word; Shift: TShiftState);

 procedure SetChanUpDown(UpDown: TUpDown);
 begin
   case Key of
     VK_UP: if UpDown.Position < 2 then UpDown.Position := UpDown.Position + 1;
     VK_DOWN: if UpDown.Position > 0 then UpDown.Position := UpDown.Position - 1;
...
begin
 if Sender = Edit6 then
  begin
   ...
   Edit6.Text := char(Ord('A') + UpDown6.Position);
  end
 else
  begin
   SetChanUpDown(UpDown7);
   Edit7.Text := char(Ord('A') + UpDown7.Position);
  end;
end;
